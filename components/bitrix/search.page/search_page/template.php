				...
				<div id="search_params" class="search-filter" style="display:<?echo $arResult["REQUEST"]["FROM"] || $arResult["REQUEST"]["TO"] || $arResult["REQUEST"]["WHERE"]? 'block': 'none'?>">
					...
					<table class="search-filter" cellspacing="0"><tbody>
						...
						<tr>
							...
							<td class="search-filter-field"><input class="btn btn-primary" value="<?echo GetMessage("CT_BSP_GO")?>" type="submit"></td>
						</tr>
					</tbody></table>
				</div>
			<?else:?>
...
<?if(isset($arResult["REQUEST"]["ORIGINAL_QUERY"])):
	...
	<?elseif(count($arResult["SEARCH"])>0):?>
		...
		<?foreach($arResult["SEARCH"] as $arItem):?>
			<div class="news_item">
				<a href="<?echo $arItem["URL"]?>"><h4><?echo $arItem["TITLE_FORMATED"]?></h4></a>
